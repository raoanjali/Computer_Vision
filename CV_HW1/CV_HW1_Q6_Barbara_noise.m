...
R = imread('Barbara_noise.bmp');
G = Gaussian(11,2); %Calling Gaussian function
...
imshow(uint8(K));title ('Canny Enhancer used');
% X = [I]+[H];
% figure,imshow(X);
...
figure, imshow((K1));title('Before non- maximum suppression')
...
figure, imshow(U);title ('Non Max Supression used')
